...
export class GroupComponent implements OnInit {
  ...
  trackId = (_index: number, item: IGroup): string => this.groupService.getGroupIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    const userName = this.sessionStorageService.retrieve('account')?.login;
    this.groups = dataFromBody.filter((item: any) => userName === 'admin' || item?.createdBy === userName);
  }

  protected fillComponentAttributesFromResponseBody(data: IGroup[] | null): IGroup[] {
    const groupsNew = this.groups ?? [];
    if (data) {
      for (const d of data) {
        if (groupsNew.map(op => op.id).indexOf(d.id) === -1) {
          groupsNew.push(d);
        }
      }
    }
